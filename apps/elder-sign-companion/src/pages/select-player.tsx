...
import { GAME_PATH } from "../utils/constants";
...
export default function SelectPlayer() {
    ...
    useEffect(() => {
        ...
        socket.on("player-added", (player) => {
            navigate(`${GAME_PATH}?roomId=${roomId}&pid=${player.id}`);
        });
    ...
    function selectPlayer(id: string) {
        if (currentPlayer) {
            navigate(`${GAME_PATH}?roomId=${roomId}&pid=${currentPlayer?.id}`);
        } else {
        ...
        <Container maxWidth="sm">
            ...
            <Stack spacing={1}>
                {players.map((player: any) => (
                    <Button
                        endIcon={currentPlayer?.id === player.id ? <Star color="warning" /> : null}
                        key={player.id}
                        variant="contained"
                        disabled={(player.selected || currentPlayer) && currentPlayer?.id !== player.id}
                        sx={{ textAlign: "center" }}
