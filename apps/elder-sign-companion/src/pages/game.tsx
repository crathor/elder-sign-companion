...
import { ExpandMore, Menu as MenuIcon, Restore, Update } from "@mui/icons-material";
...
export default function Game() {
    ...
    const { gameState, player, players, playerActions, gameActions } = useGame(params.roomId, params.pid);
        ...
        >
            <Toolbar sx={{ paddingRight: 1 }}>
                ...
                <Menu anchorEl={anchorEl} open={showMenu} onClose={handleMenuClose}>
                    <Typography sx={{ p: 1, textAlign: "center", width: "100%" }} component="div" variant="button">
                        Room ID: {params.roomId}
                    </Typography>

                    <Divider />
                    <MenuItem onClick={gameActions.handleChangePlayer}>Change Character</MenuItem>
                    <MenuItem onClick={gameActions.handleLeaveGame}>Leave Game</MenuItem>
                </Menu>
            </Toolbar>
            ...
            <Stack direction="row" spacing={1} p={1}>
                <PlayerAllyButton
                    player={player}
                    onClick={player.hasAlly ? playerActions.addAllyToken : playerActions.addAlly}
                    onRemoveClick={playerActions.removeAlly}
                />
                {player.hasDailyAbility && <PlayerAbilityButton player={player} onClick={handleUsePlayerAbility} />}
            </Stack>
            <Stack direction="row" spacing={1}>
                <PlayerStat
                    name="Clue Tokens"
            ...
            <PlayerDrawer open={showPlayers} onClose={togglePlayersDrawer} players={players} />
            <PlayerHealDialog
                open={showHealModal}
